...
We have all the data saved, so we'll just load them in here.
...
```
The pitchers however, gave us interesting results, for the cutoff that gives us the lowest average error is anything at or above 0.92.
```{r}
...
Back to the pitcher cutoff value: in order to have a more interesting predictor, we decided to use the next best cutoff, which was around 0.4
```{r}
...
glm.fit.pitch
```
For the hitters it seems like Triples per plate appearance had the largest coefficient in the regression, followed by batting average.
