...
The cutoff that gives us the lowest average error is anything at or above 0.93.
```{r}
cutoffs <- crossValid(players.pitchers.stats)
which(cutoffs == min(cutoffs))/100
```
This cutoff actually resulted in a prediction of 0 every single time, which effectively told us that these variables don't tell us much about how likely it is for a pitcher to be traded.
However, so that we can have a more interesting predictor, we'll use the next best cutoff.
```{r}
x = cutoffs[1:92]
which(x == min(x))/100
```
Given this we'll use 0.5 for the pitchers.
...
For pitchers the most influential was Shutouts per game, followed by W-L%.
